...
But the API is unstable and if an error occurs while creating or deleting groups the operation should be rollbacked.
...
When background job works, it gets transactions from groupdelete table and sends delete request to the cluster API. 
...
There is also a producer (sender) application for API with Pika library to send rollback transaction operations to the cluster. 
