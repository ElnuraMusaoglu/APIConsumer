...
The Rest API takes parameters to create, get or delete the groups and sends an HTTP request to the cluster API. 
...
and sends a delete request to the cluster API. 
If the cluster API is still unstable, the operation is postponed to be done later. Otherwise, 
if the cluster API is stable, the delete operation is carried out. 
If data doesn't exist in the cluster API or is deleted successfully, 
the cluster API sends 200 or 204 HTTP responses and the  API deletes the transaction from groupdelete table, 
and in this way, rollback operation is carried out successfully.
...
In cluster API there is a consumer (receiver) to get transactions and deletes the groups if exist via AMQP. 
But the message broker rollback operations were not used to send the transactions via HTTP.
